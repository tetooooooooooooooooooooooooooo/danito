...
def main():
    ...
    print("\n=== the front page says nothing about how big this is ===")
    global RUNTIME
    assert not RUNTIME, "this suite has run so far with no heartbeat at all"
    import datetime
    # The heartbeat carries a server and a member count, and the front page used to show both
    # under the hero. They were taken off deliberately, so the check is that a live heartbeat
    # with real figures in it still puts nothing on the page.
    RUNTIME.update({"_id": "bot", "guild_count": 7, "member_count": 104382,
                    "last_seen": datetime.datetime.now(datetime.timezone.utc)})
    body = html.unescape(c.get("/").data.decode())
    assert 'class="tallies"' not in body
    for gone in ("Members watched", "104k", "104382"):
        assert gone not in body, gone
    RUNTIME.clear()
    print("  a heartbeat with 7 servers and 104382 members reaches the page as nothing OK")

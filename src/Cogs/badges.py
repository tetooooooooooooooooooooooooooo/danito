...
class Badges(commands.Cog):
    ...
    async def cbc(self, interaction: discord.Interaction, badge: str):
        ...
        if badge == "all":
            ...
            text = "\n".join(
                f"{badge_attrs[b]}: {c}" for b, c in result.items()
        ...
        if badge not in badge_attrs:
            await interaction.followup.send("Unknown badge. Please select one from the autocomplete list.")
            return

        count = sum(self.has_badge(m, badge) for m in members)
        await interaction.followup.send(f"**{badge_attrs[badge]}:** {count} members")
    ...
    async def cbu(self, interaction: discord.Interaction, badge: str):
        ...
        guild = interaction.guild
        members = [m for m in guild.members if self.has_badge(m, badge)]
        ...
        await interaction.followup.send(f"**Users with {badge_attrs[badge]}:**\n{names}")
